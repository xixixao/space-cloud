...
(function() {
  var Answer, CommentA, CommentQ, Course, File, Q, Question, User, _ref;
    ...
    app.get('/users/:login', function(request, response) {
      return User.findOne({
        ...
        if (err != null) {
          ...
          return user.populate('courses.code', function(err, user) {
            var coursePermissions, files;

            user = user.toObject();
            coursePermissions = user.courses;
            files = Q.map(coursePermissions, function(_arg) {
              var code, courseId;
